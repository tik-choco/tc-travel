...
// The vendored .wasm is committed, and rustc embeds absolute source paths
// (home dir + username) in panic-location metadata — remap them so no
// machine-local paths leak into the repository.
const homeDir = process.env.HOME || process.env.USERPROFILE || "";
const remapFlags = homeDir ? `--remap-path-prefix=${homeDir}=/build` : "";
run(wasmPackBin, ["build", "--target", "web", "--release"], wasmCrateDir, {
  RUSTFLAGS: [process.env.RUSTFLAGS, remapFlags].filter(Boolean).join(" "),
});

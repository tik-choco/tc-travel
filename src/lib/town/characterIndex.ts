...
export interface CharacterIndexEntry {
  ...
  summary: string;
  /** Fully compiled system-prompt persona text (Japanese sections). */
...
function toEntry(value: unknown): CharacterIndexEntry | null {
  ...
  if (!id || !name || !summary || !personaPrompt || !updatedAt) return null;

  const entry: CharacterIndexEntry = { id, name, summary, personaPrompt, updatedAt };
